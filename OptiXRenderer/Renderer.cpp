...
void Renderer::initPrograms()
{
    ...
    programs["analyticdirect"] = createProgram("Analytic.cu", "closestHit");
...
void Renderer::buildScene()
{
    ...
    currentFrame = 0;
    numFrames = scene->samplesPerPixel;
    ...
    programs["integrator"]["lightStratify"]->setInt(scene->lightStratify ? 1 : 0);
    programs["integrator"]["reduceVariance"]->setInt(scene->reduceVariance ? 1 : 0);

    programs["integrator"]["NEE"]->setInt(scene->NEE);
    programs["integrator"]["RR"]->setInt(scene->RR);
